...
namespace eMandates.Merchant.Library.Configuration
{
    ...
    public class Configuration : IConfiguration
    {
        ...
        /// <summary>
        /// Gets the status of the configuration initialization.
        /// </summary>
        ...
        public static void Load()
        {
            try
            {
                ...
                if (!Boolean.TryParse(config["eMandates.ServiceLogs.Enabled"], out serviceLogsEnabled))
                    serviceLogsEnabled = true;

                string eMandateContractSubIdString = config["eMandates.Contract.SubId"];
                uint eMandateContractSubId = 0;
                if (!string.IsNullOrWhiteSpace(eMandateContractSubIdString))                    
                {
                    uint.TryParse(eMandateContractSubIdString, out eMandateContractSubId);                    
                }

                Instance = new Configuration(
                        eMandateContractId: config["eMandates.Contract.Id"],
                        merchantReturnUrl: config["eMandates.Merchant.ReturnUrl"],
                        signingCertificateFingerprint: config["eMandates.SigningCertificate.Thumbprint"],
                        acquirerCertificateFingerprint: config["eMandates.AcquirerCertificate.Thumbprint"],
                        acquirerAlternateCertificateFingerprint: config["eMandates.AcquirerAlternateCertificate.Thumbprint"],
                        acquirerUrlDirectoryReq: config["eMandates.Acquirer.DirectoryRequestUrl"],
                        acquirerUrlTransactionReq: config["eMandates.Acquirer.TransactionRequestUrl"],
                        acquirerUrlStatusReq: config["eMandates.Acquirer.StatusRequestUrl"],
                        serviceLogsEnabled: serviceLogsEnabled,
                        serviceLogsLocation: config["eMandates.ServiceLogs.Location"],
                        serviceLogsPattern: config["eMandates.ServiceLogs.Pattern"],
                        eMandateContractSubId: eMandateContractSubId
                    );
            }
            catch (Exception e)
            {
